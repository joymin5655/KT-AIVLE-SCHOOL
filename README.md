...
검증: JS 전처리 벡터는 파이썬 golden과 오차 **5e-6**, ONNX 추론은 원본 `.pkl`과 **라벨·확률이 일치**합니다. (랜드마크 추정 모델의 미세 차이로 실제 예측이 원본과 완전히 같지는 않을 수 있음.)

**프라이버시·보안 설계:**
- 영상·랜드마크·판정은 브라우저 밖으로 **전혀 나가지 않음** — 전송·저장·녹화 코드 없음.
- 모든 ML 자산(onnxruntime-web · MediaPipe · 포즈 모델)을 `live/vendor/`에 **벤더링**해, 페이지가 제3자 CDN에 요청을 **하나도 보내지 않음**(호스트 GitHub 외 접속 흔적 0).
- **CSP**(`connect-src 'self'`)로, 설령 코드가 변조돼도 브라우저가 외부로의 데이터 전송을 차단. 인라인 스크립트 제거(`app.js` 분리)로 스크립트 인젝션 표면 축소.
